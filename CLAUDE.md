...
- **Persistence**: MySQL (`spring.jpa.hibernate.ddl-auto=update`), local dev connects to `localhost:3306/maeumsumgil` (auto-created) as `root`/`1234` — not production-ready credentials. Entity definitions ARE the schema (no migration tool); see `PROGRESS.md` for the current derived DDL.
